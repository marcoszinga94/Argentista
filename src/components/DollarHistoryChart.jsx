...
const DollarHistoryChart = () => {
  ...
  const fetchDollarData = async () => {
    ...
    const startDate = timeRange >= 1 ? subMonths(endDate, timeRange) : subWeeks(endDate, Math.floor(timeRange * 4));
  ...
  const fetchDataForDateRange = async (startDate, endDate) => {
    ...
    while (currentDate <= endDate) {
      ...
      try {
        const response = await fetch(`https://api.argentinadatos.com/v1/cotizaciones/dolares/${selectedCasa}/${formattedDate}`);

        if (response.ok) {
          const dayData = await response.json();
          data.push({ date: formattedDate, ...dayData });
        } else {
          console.warn(`No hay datos disponibles para la fecha: ${formattedDate}`);
        }
      } catch (error) {
        console.error(`Error al obtener datos para la fecha ${formattedDate}:`, error);
      }
  ...
  const prepareChartData = (data) => {
    if (data.length === 0) {
      throw new Error("No se encontraron datos en el rango de fechas seleccionado.");
    }

    const labels = data.map((item) => format(new Date(item.date), "d MMM", { locale: es }));
  ...
  if (!chartData || chartData?.datasets[0]?.data.length === 0) {
    return (
      <div className="bg-white p-6 rounded-lg shadow-md w-full">
        <div className="text-center">No hay datos disponibles para el rango seleccionado.</div>
      </div>
    );
  }
    ...
    <div className="bg-white p-6 rounded-lg shadow-md w-full">
      ...
      <div className="mb-4 flex space-x-4">
        <div>
          ...
          <select id="casa-select" value={selectedCasa} onChange={(e) => setSelectedCasa(e.target.value)} className="p-2 border rounded">
        ...
        <div>
          ...
          <select id="time-range-select" value={timeRange} onChange={(e) => setTimeRange(parseFloat(e.target.value))} className="p-2 border rounded">
