...
const InflationCalculator = () => {
  ...
  useEffect(() => {
    if (inflationData.length > 0) {
      const latestDate = new Date(inflationData[inflationData.length - 1].fecha);
  ...
  const calculateInflation = () => {
    ...
    const startIndex = inflationData.findIndex((item) => item.fecha >= startDate);
    ...
    const inflationFactor = relevantData.reduce((acc, item) => acc * (1 + item.valor / 100), 1);
    const adjustedAmount = parseFloat(amount) * inflationFactor;

    const totalInflation = (adjustedAmount / parseFloat(amount) - 1) * 100;
    const monthsDiff = endIndex - startIndex;
    const annualizedInflation = Math.pow(1 + totalInflation / 100, 12 / monthsDiff) - 1;
    const averageMonthlyInflation = (Math.pow(1 + annualizedInflation, 1 / 12) - 1) * 100;
  ...
  const years = Array.from({ length: 30 }, (_, i) => (new Date().getFullYear() - 29 + i).toString()).reverse();
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-md">
      <h2 className="text-2xl font-semibold mb-4 text-center">Calculadora de Inflación</h2>
      <div className="flex flex-col gap-4">
        <div className="flex items-center gap-2 m-auto">
          <span className="text-xl">$</span>
          <input type="number" value={amount} onChange={(e) => setAmount(e.target.value)} className="w-32 p-2 border rounded" />
          <span className="text-xl">en</span>
          <select value={startMonth} onChange={(e) => setStartMonth(e.target.value)} className="p-2 border rounded">
        ...
        <div className="flex items-center gap-2 m-auto">
          <span className="text-xl">$</span>
          <input type="text" value={result ? result.adjustedAmount : ""} readOnly className="w-32 p-2 border rounded bg-gray-100" />
          <span className="text-xl">en</span>
          <select value={endMonth} onChange={(e) => setEndMonth(e.target.value)} className="p-2 border rounded">
            {months.map((month) => (
              <option key={month.value} value={month.value}>
                {month.label}
              </option>
        ...
        {result && (
          <div className="mt-4 p-4 bg-gray-100 rounded">
            <p className="text-center text-pretty max-w-[400px]">
              Esto representa un incremento del <strong>{result.totalInflation}%</strong>, es decir, un incremento promedio del{" "}
              <strong>{result.averageMonthlyInflation}%</strong> por mes (<strong>{result.annualizedInflation}%</strong> anualizado).
            </p>
          </div>
        )}
      </div>
